...
const FloatingMicButton: FC<{
  ...
  return (
    ...
    >
      <MicIcon className={`w-5 h-5 ${isRecording ? 'text-[#EF4444]' : 'text-white'}`} />
    </button>
  );
};
...
export const TerminalPanel: FC<TerminalPanelProps> = ({
  ...
  return (
    <div
      className={`border-t border-[#1E1E26] flex flex-col ${fillContainer ? 'flex-1 min-h-0 border-t-0 bg-[#0A0A0F]' : 'shrink-0'} ${isFullscreen ? 'absolute inset-0 z-50 border-t-0 bg-[#0A0A0F]' : ''}`}
    ...
    >
      ...
      {visible && !isFullscreen && (
        ...
        >
          <div className="w-10 h-1 rounded-full bg-[#1E1E26] group-hover:bg-[#6366F1] group-hover:h-1.5 transition-all" />
        </div>
      )}
      {/* Header with tabs */}
      <div className="flex items-center bg-[#141419] select-none shrink-0 border-b border-[#1E1E26]/50">
        <div
          className="flex items-center gap-2 px-3 py-1.5 text-sm text-[#8B8B9E] cursor-pointer hover:bg-[#1A1A22]/50 transition-colors"
          onClick={onToggle}
          role="button"
        ...
        >
          ...
          <ChevronIcon expanded={visible} className="w-3 h-3 text-[#55556A]" />
        </div>
              ...
              return (
                <div
                  ...
                  className={`group px-2 py-1.5 text-xs font-medium whitespace-nowrap rounded-t transition-colors flex items-center gap-1 cursor-pointer ${isActive
                    ? 'bg-[#0A0A0F] text-[#6366F1] border-t border-l border-r border-[#1E1E26]'
                    : isActivated
                      ? 'text-[#8B8B9E] hover:bg-[#1A1A22]/50'
                      : 'text-[#55556A] hover:bg-[#1A1A22]/50 hover:text-[#8B8B9E]'
                    }`}
                  onClick={() => {
                ...
                >
                  ...
                  <span>{tab.name}</span>
                  {tab.isDuplicate && (
                    <span className="text-[9px] text-[#55556A] font-mono">{t('terminal.duplicate')}</span>
                  )}
                  {isActivated && (
                    <span
                      className="w-5 h-5 ml-1 flex items-center justify-center rounded-full hover:bg-[#1E1E26] text-[#55556A] hover:text-[#8B8B9E] transition-colors"
            ...
            <div className="flex items-center gap-1 shrink-0 mr-1">
              {onOpenInEditor && activePath && (
                <button
                  ...
                  className="p-1 rounded text-[#8B8B9E] hover:text-blue-300 hover:bg-[#1A1A22]/60 transition-colors"
          ...
          <div className="flex items-center mx-1">
            {activatedTerminals.size >= 2 && (
              <button
                ...
                className="p-1.5 hover:bg-[#1A1A22] rounded text-[#55556A] hover:text-[#8B8B9E] transition-colors"
      ...
      >
        ...
        {mountedTerminals.size > 0 ? (
          <>
            ...
            {!activeTerminalTab && (
              <div className="flex items-center justify-center h-full text-[#55556A] text-sm">
        ...
        {showAltVHint && !IS_MOBILE && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 px-3 py-1.5 bg-[#141419]/90 border border-[#1E1E26]/50 rounded-lg text-xs text-[#8B8B9E] shadow-lg pointer-events-none animate-in fade-in duration-200">
            {t('terminal.altVHint')}
          </div>
        )}
          ...
          <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-4 fade-in-0 p-4">
            ...
            {staging && (staging.rawText || staging.interimText) && (
              <div className="w-full max-w-md flex flex-col gap-2">
                {/* 原始语音 */}
                <div className="rounded-lg bg-[#141419]/80 border border-[#1E1E26]/50 p-3">
                  <div className="text-[10px] text-[#55556A] mb-1 font-medium uppercase tracking-wider">{t('voice.rawText')}</div>
                  <div className="text-sm text-[#E8E8ED] leading-relaxed break-all">
                    {staging.rawText}
                    {staging.interimText && (
                      <span className="text-[#55556A] italic">{staging.interimText}</span>
                    )}
                  </div>
                </div>
                ...
                {(staging.refinedText || staging.isRefining || staging.refineFailed) && (
                  <div className={`rounded-lg p-3 ${staging.refineFailed
                    ...
                    <div className="text-[10px] mb-1 font-medium uppercase tracking-wider flex items-center gap-1">
                      <span className={staging.refineFailed ? 'text-[#EF4444]' : 'text-[#6366F1]'}>
                        {t('voice.refinedText')}
                      </span>
                      {staging.isRefining && (
                        <span className="text-[#55556A] animate-pulse">{t('voice.refining')}</span>
                      )}
                    </div>
            ...
            <span className="text-sm text-[#8B8B9E] select-none">
        ...
        {/* 移动端 Web 录音指示器 + 暂存区卡片 */}
        {voiceStatus === 'recording' && IS_MOBILE_WEB && (
          <div className="absolute bottom-20 left-2 right-2 z-15 rounded-xl bg-[#141419]/95 border border-[#1E1E26]/50 p-3 shadow-2xl backdrop-blur-sm">
            {/* 录音中标题 — 始终显示 */}
            <div className="flex items-center gap-2 mb-1">
              <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
              <span className="text-[10px] text-[#EF4444] font-medium uppercase tracking-wider">{t('terminal.recordingHint')}</span>
            </div>
            ...
            {staging && (staging.rawText || staging.interimText) && (
              <>
                <div className="text-[10px] text-[#55556A] mb-1 mt-2 font-medium uppercase tracking-wider">{t('voice.rawText')}</div>
                <div className="text-sm text-[#E8E8ED] leading-relaxed break-all mb-2">
                  {staging.rawText}
                  {staging.interimText && (
                    <span className="text-[#55556A] italic">{staging.interimText}</span>
                  )}
