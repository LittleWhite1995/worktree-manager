  ...
  return (
    <button
      ...
      className={`z-20 w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-sm touch-none ${isRecording
        ? 'bg-[--color-error]/70 border-2 border-[--color-error] shadow-[0_0_12px_rgba(239,68,68,0.5)] animate-pulse'
        : 'bg-[--color-bg-surface]/70 border-2 border-green-500/60'
        }`}
      style={style}
    ...
    >
      <MicIcon className={`w-5 h-5 ${isRecording ? 'text-[--color-error]' : 'text-white'}`} />
    </button>
  );
};
...
function getVoiceButtonClass(voiceStatus: VoiceStatus, voiceWarning?: string | null): string {
  switch (voiceStatus) {
    case 'recording': return 'text-[--color-error] hover:bg-[--color-error]/10';
    case 'ready': return 'text-green-400 hover:bg-green-900/30';
    case 'error': return voiceWarning ? 'text-yellow-400 hover:bg-[--color-bg-elevated]' : 'text-[--color-error] hover:bg-[--color-bg-elevated]';
    default: return 'text-[--color-text-muted] hover:text-[--color-text-secondary] hover:bg-[--color-bg-elevated]';
  }
}
...
export const TerminalPanel: FC<TerminalPanelProps> = ({
  ...
  return (
    <div
      className={`border-t border-[--color-border] flex flex-col ${fillContainer ? 'flex-1 min-h-0 border-t-0 bg-[--color-bg-base]' : 'shrink-0'} ${isFullscreen ? 'absolute inset-0 z-50 border-t-0 bg-[--color-bg-base]' : ''}`}
    ...
    >
      ...
      {visible && !isFullscreen && (
        ...
        >
          <div className="w-10 h-1 rounded-full bg-[--color-border] group-hover:bg-[--color-accent] group-hover:h-1.5 transition-all" />
        </div>
      )}
      {/* Header with tabs */}
      <div className="flex items-center bg-[--color-bg-surface] select-none shrink-0 border-b border-[--color-border]/50">
        <div
          className="flex items-center gap-2 px-3 py-1.5 text-sm text-[--color-text-secondary] cursor-pointer hover:bg-[--color-bg-elevated]/50 transition-colors"
          onClick={onToggle}
          role="button"
        ...
        >
          ...
          <ChevronIcon expanded={visible} className="w-3 h-3 text-[--color-text-muted]" />
        </div>
              ...
              return (
                <div
                  ...
                  className={`group px-2 py-1.5 text-xs font-medium whitespace-nowrap rounded-t transition-colors flex items-center gap-1 cursor-pointer ${isActive
                    ? 'bg-[--color-bg-base] text-[--color-accent] border-t border-l border-r border-[--color-border]'
                    : isActivated
                      ? 'text-[--color-text-secondary] hover:bg-[--color-bg-elevated]/50'
                      : 'text-[--color-text-muted] hover:bg-[--color-bg-elevated]/50 hover:text-[--color-text-secondary]'
                    }`}
                  onClick={() => {
                ...
                >
                  ...
                  <span>{tab.name}</span>
                  {tab.isDuplicate && (
                    <span className="text-[9px] text-[--color-text-muted] font-mono">{t('terminal.duplicate')}</span>
                  )}
                  {isActivated && (
                    <span
                      className="w-5 h-5 ml-1 flex items-center justify-center rounded-full hover:bg-[--color-border] text-[--color-text-muted] hover:text-[--color-text-secondary] transition-colors"
            ...
            <div className="flex items-center gap-1 shrink-0 mr-1">
              {onOpenInEditor && activePath && (
                <button
                  ...
                  className="p-1 rounded text-[--color-text-secondary] hover:text-blue-300 hover:bg-[--color-bg-elevated]/60 transition-colors"
              ...
              <button
                className={`terminal-badge-marquee text-xs px-2.5 py-0.5 rounded-md max-w-[160px] overflow-hidden cursor-pointer transition-colors ${
                  badgeCopied
                    ...
                    : 'text-[--color-accent] bg-[--color-accent]/20 border border-[--color-accent]/30 hover:bg-[--color-accent]/30 hover:text-[--color-accent] hover:border-[--color-accent]/50 active:bg-[--color-accent]/40'
                }`}
                onClick={() => {
          ...
          <div className="flex items-center mx-1">
            {activatedTerminals.size >= 2 && (
              <button
                ...
                className="p-1.5 hover:bg-[--color-bg-elevated] rounded text-[--color-text-muted] hover:text-[--color-text-secondary] transition-colors"
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors ${
                searchOpen ? 'text-[--color-accent] bg-[--color-accent]/10' : 'text-[--color-text-muted] hover:text-[--color-text-secondary] hover:bg-[--color-bg-elevated]'
              }`}
      ...
      >
        ...
        {/* 语音错误提示（红色） */}
        {showError && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 px-4 py-2 bg-[--color-error]/10 border border-[--color-error]/30 rounded-lg text-sm text-[--color-error] shadow-lg animate-in fade-in slide-in-from-top-2 duration-200">
            {showError}
          </div>
        )}
        ...
        {showWarning && !showError && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 px-4 py-2 bg-[--color-warning]/10 border border-[--color-warning]/30 rounded-lg text-sm text-[--color-warning] shadow-lg animate-in fade-in slide-in-from-top-2 duration-200">
            {showWarning}
          </div>
        )}
        ...
        {showAltVHint && !IS_MOBILE && (
          <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 px-3 py-1.5 bg-[--color-bg-surface]/90 border border-[--color-border]/50 rounded-lg text-xs text-[--color-text-secondary] shadow-lg pointer-events-none animate-in fade-in duration-200">
            {t('terminal.altVHint')}
          </div>
        )}
          ...
          <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-4 fade-in-0 p-4">
            ...
            {staging && (staging.rawText || staging.interimText) && (
              <div className="w-full max-w-md flex flex-col gap-2">
                {/* 原始语音 */}
                <div className="rounded-lg bg-[--color-bg-surface]/80 border border-[--color-border]/50 p-3">
                  <div className="text-[10px] text-[--color-text-muted] mb-1 font-medium uppercase tracking-wider">{t('voice.rawText')}</div>
                  <div className="text-sm text-[--color-text-primary] leading-relaxed break-all">
                    {staging.rawText}
                    {staging.interimText && (
                      <span className="text-[--color-text-muted] italic">{staging.interimText}</span>
                    )}
                  </div>
                </div>
                ...
                {(staging.refinedText || staging.isRefining || staging.refineFailed) && (
                  <div className={`rounded-lg p-3 ${staging.refineFailed
                    ? 'bg-[--color-error]/10 border border-[--color-error]/30'
                    : 'bg-[--color-bg-surface]/80 border border-[--color-accent]/40'
                    }`}>
                    <div className="text-[10px] mb-1 font-medium uppercase tracking-wider flex items-center gap-1">
                      <span className={staging.refineFailed ? 'text-[--color-error]' : 'text-[--color-accent]'}>
                        {t('voice.refinedText')}
                      </span>
                      {staging.isRefining && (
                        <span className="text-[--color-text-muted] animate-pulse">{t('voice.refining')}</span>
                      )}
                    </div>
                    <div className="text-sm leading-relaxed break-all">
                      {staging.refineFailed ? (
                        <span className="text-[--color-error]/80">{t('voice.refineFailed')}</span>
                      ) : (
                        <span className="text-[--color-accent]">{staging.refinedText}</span>
                      )}
                    </div>
                  </div>
                )}
            ...
            <span className="text-sm text-[--color-text-secondary] select-none">
        ...
        {/* 移动端 Web 录音指示器 + 暂存区卡片 */}
        {voiceStatus === 'recording' && IS_MOBILE_WEB && (
          <div className="absolute bottom-20 left-2 right-2 z-15 rounded-xl bg-[--color-bg-surface]/95 border border-[--color-border]/50 p-3 shadow-2xl backdrop-blur-sm">
            {/* 录音中标题 — 始终显示 */}
            <div className="flex items-center gap-2 mb-1">
              <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
              <span className="text-[10px] text-[--color-error] font-medium uppercase tracking-wider">{t('terminal.recordingHint')}</span>
            </div>
            ...
            {staging && (staging.rawText || staging.interimText) && (
              <>
                <div className="text-[10px] text-[--color-text-muted] mb-1 mt-2 font-medium uppercase tracking-wider">{t('voice.rawText')}</div>
                <div className="text-sm text-[--color-text-primary] leading-relaxed break-all mb-2">
                  {staging.rawText}
                  {staging.interimText && (
                    <span className="text-[--color-text-muted] italic">{staging.interimText}</span>
                  )}
