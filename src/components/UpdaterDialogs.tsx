...
const SimpleMarkdown: FC<{ content: string }> = ({ content }) => {
  ...
  const renderInline = (text: string): React.ReactNode => {
    ...
    while (remaining.length > 0) {
      ...
      if (boldIdx <= codeIdx && boldMatch) {
        ...
        parts.push(<code key={partKey++} className="px-1 py-0.5 bg-[--color-bg-elevated] rounded text-xs text-[--color-accent]">{codeMatch[1]}</code>);
  ...
  for (const line of lines) {
    ...
    if (headingMatch) {
      ...
      if (level <= 2) {
        elements.push(
          <h3 key={key++} className="text-sm font-semibold text-[--color-text-primary] mb-2 mt-3 first:mt-0">
            {renderInline(text)}
          </h3>
        );
      } else {
        elements.push(
          <h4 key={key++} className="text-sm font-medium text-[--color-text-secondary] mb-1.5 mt-2.5 first:mt-0">
            {renderInline(text)}
          </h4>
        );
      }
    ...
    flushList();
    elements.push(
      <p key={key++} className="text-sm text-[--color-text-secondary] mb-2">
      ...
      <DialogContent className="max-w-[500px] p-0">
        ...
        <div className="px-5 py-4">
          <DialogDescription asChild>
            <div>
              <p className="text-lg font-medium text-[--color-text-primary] mb-3">
              ...
              {updateInfo.notes.length > 0 && (
                <div className="mb-4 max-h-[300px] overflow-y-auto pr-1">
                  <p className="text-sm font-medium text-[--color-text-secondary] mb-2">{t('updater.releaseNotes')}</p>
                  <div className="pl-1">
        ...
        <DialogFooter className="p-5 pt-0 flex-col gap-2 sm:flex-col">
          ...
          <Button
            variant="outline"
            className="w-full text-[--color-text-secondary] hover:text-[--color-text-primary]"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[450px] p-0" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader className="p-5 pb-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[--color-accent]/10 flex items-center justify-center">
              <Download className="w-5 h-5 text-[--color-accent] animate-pulse" />
            </div>
            <DialogTitle className="text-xl">
        ...
        <div className="px-5 py-4">
          <DialogDescription asChild>
            <div>
              ...
              {/* Progress Bar */}
              <div className="mb-4">
                <div className="relative h-3 w-full overflow-hidden rounded-full bg-[--color-bg-elevated]">
                  <div
                    className="h-full bg-gradient-to-r from-[--color-accent] to-[--color-accent] transition-all duration-300 ease-in-out rounded-full"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[450px] p-0">
        ...
        <div className="px-5 py-4">
          <DialogDescription asChild>
            <div className="text-center">
              <p className="text-lg text-[--color-text-primary] mb-2">
        ...
        <DialogFooter className="p-5 pt-0 flex-row gap-3 sm:flex-row">
          ...
          <Button
            className="flex-1 bg-[--color-success] hover:bg-[--color-success]/80 group"
            onClick={onRestart}
          >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[500px] p-0">
        ...
        <div className="px-5 py-4">
          <DialogDescription asChild>
            <div>
              <p className="text-[--color-text-secondary] mb-3">{t('updater.errorOccurred')}</p>

              <div className="p-3 bg-[--color-bg-base] border border-[--color-border] rounded-lg mb-4">
              ...
              <div className="space-y-1.5">
                <p className="text-sm font-medium text-[--color-text-secondary]">{t('updater.youCan')}</p>
...
export const UpToDateToast: FC<UpToDateToastProps> = ({ show }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] animate-in slide-in-from-bottom-4 fade-in duration-300">
      <div className="flex items-center gap-3 bg-[--color-bg-surface] border border-[--color-border] rounded-lg p-4 shadow-lg">
        <div className="w-8 h-8 rounded-lg bg-green-500/10 flex items-center justify-center">
          <CheckCircle className="w-4 h-4 text-[--color-success]" />
        </div>
        <div>
          <p className="text-sm font-medium text-[--color-text-primary]">{t('updater.upToDate')}</p>
