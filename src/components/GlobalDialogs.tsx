          ...
          <div className="py-2 space-y-2">
            {[
              ...
              <div key={keys} className="flex items-center justify-between py-1.5 px-1">
                <span className="text-sm text-[#8B8B9E]">{desc}</span>
                <div className="flex gap-1">
                  {keys.split(' ').map((k) => (
                    <kbd key={k} className="px-2 py-0.5 bg-[#1A1A22] border border-[#1E1E26] rounded text-xs font-mono text-[#8B8B9E]">{k}</kbd>
                  ))}
                </div>
              </div>
            ))}
        ...
        <DialogContent className="max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <a href="https://dashboard.ngrok.com/get-started/your-authtoken" target="_blank" rel="noopener noreferrer" className="text-[#6366F1] hover:underline">{t('settings.ngrokGetToken')}</a>
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Input
          ...
          <DialogFooter className="flex items-center justify-between sm:justify-between">
            <button
              type="button"
              className="text-xs text-[#6366F1] hover:text-blue-300 underline transition-colors"
              onClick={() => {
        ...
        <DialogContent className="max-w-[440px]">
          <DialogHeader>
            ...
            <DialogDescription className="space-y-3 pt-2 text-left">
              ...
              <p className="font-medium text-[#8B8B9E]">🔒 {t('share.disclaimerEncryption', '所有分享连接均通过加密通道传输（HTTPS/WSS），隧道服务不会存储或截取任何数据，您的代码和工作内容始终受到端到端加密保护。')}</p>
              <p className="text-[#55556A] text-xs">{t('share.disclaimerResponsibility', '使用分享功能即代表您已知悉上述信息并自行承担相关合规责任。')}</p>
            </DialogDescription>
          </DialogHeader>
