    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[480px] p-0">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[--color-text-secondary] mb-2">{t('addWorkspace.dirPath')}</label>
            ...
            <p className="text-xs text-[--color-text-muted] mt-1">{t('addWorkspace.dirPathHint')}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-[--color-text-secondary] mb-2">{t('common.name')}</label>
            <Input
              type="text"
              value={name}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[480px] p-0">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[--color-text-secondary] mb-2">{t('addWorkspace.workspaceName')}</label>
            <Input
              type="text"
              value={name}
              onChange={(e) => onNameChange(e.target.value)}
          ...
          <div>
            <label className="block text-sm font-medium text-[--color-text-secondary] mb-2">{t('addWorkspace.parentDir')}</label>
            <div className="flex gap-2">
              <Input
                type="text"
                value={path}
          ...
          {fullPath && (
            <div className="p-3 rounded bg-[--color-bg-surface] border border-[--color-border]/50">
              <p className="text-xs text-[--color-text-secondary] mb-1">{t('addWorkspace.willCreate')}</p>
              <p className="text-sm font-mono text-[--color-text-secondary] select-text">{fullPath}</p>
            </div>
          )}
        </div>
        <DialogFooter className="p-5 border-t border-[--color-border]">
