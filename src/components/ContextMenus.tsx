...
export const WorktreeContextMenu: FC<ContextMenuProps> = ({
  ...
  return (
    ...
    >
      <div
        className="absolute bg-[--color-bg-surface] border border-[--color-border] rounded-lg shadow-lg py-1 min-w-[140px]"
        style={{ left: x, top: y }}
      ...
      >
        ...
        <button
          onClick={onArchive}
          className="w-full px-3 py-1.5 text-left text-sm text-[--color-text-primary] hover:bg-[--color-bg-elevated] flex items-center gap-2 transition-colors"
        >
...
const AppPickerPopover: FC<AppPickerPopoverProps> = ({ anchorRect, items, onSelect, onClose, renderIcon }) => {
  ...
  return createPortal(
    <div
      ref={menuRef}
      className="fixed z-[9999] grid grid-cols-3 gap-1 p-1 bg-[--color-bg-surface] border border-[--color-border] rounded-lg shadow-lg"
      style={{ left, top }}
    ...
    >
      {items.map((item) => (
        <button
          ...
          className="p-1.5 rounded text-[--color-text-secondary] hover:text-[--color-text-primary] hover:bg-[--color-bg-elevated] transition-colors"
        >
          {renderIcon(item.id)}
        </button>
      ))}
